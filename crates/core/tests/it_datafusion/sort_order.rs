...
//!
//! First verify the behaviour of plain DataFusion with a `ListingTable` over
...
//! plan shapes that the Delta table provider should reproduce.
...
/// The `PushdownSort` optimizer rule in DataFusion 53 only optimizes scans
/// whose declared ordering is the *reverse* of the requested one (returning an
/// inexact result that keeps the `SortExec`); reordering files by statistics
/// at pushdown time is not supported yet. A same-direction `ORDER BY` thus
/// still requires a full sort unless `split_file_groups_by_statistics` builds
/// non-overlapping groups at planning time.
#[tokio::test]
async fn listing_table_sort_pushdown_still_requires_sort() -> TestResult<()> {
    let dir = TempDir::new()?;
    write_partitioned_table(dir.path())?;
